...
function saveReviews() { localStorage.setItem(`eai_reviews_${activeSlot}`, JSON.stringify(cachedReviews)); }
...
function setupEvents() {
  ...
  document.getElementById('customScenarioBtn')?.addEventListener('click', () => {
    const text = document.getElementById('customScenarioInput').value.trim();
    if (!text) { document.getElementById('customScenarioInput').focus(); return; }
    startRoleplay({
      id: 'custom',
      emoji: '✏️',
      title: '커스텀 롤플레이',
